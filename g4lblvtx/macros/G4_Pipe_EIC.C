...
namespace G4PIPE
{
	...
	bool use_forward_pipes = true; // This loads the conical sections of the beampipe
}  // namespace G4PIPE
		...
		double radius)
{
	...
	{
		cout << "inconsistency: radius: " << radius << " larger than pipe inner radius: " << G4PIPE::be_pipe_radius << endl;
		gSystem->Exit(-1);
	}
	...
	if (do_pipe_electron_forward_extension)
	{
		...
		gdml->set_string_param("GDMPath","detector/Detector_chamber_3-20-20.G4Import.gdml");
