...
// EicFRichDetector::ConstructMe() method
// but the convention is as mentioned cm and deg
//____________________________________________________________________________..
//
#include "EicFRichSubsystem.h"

#include "EicFRichDetector.h"
#include "EicFRichSteppingAction.h"
...
EicFRichSubsystem::EicFRichSubsystem(const std::string &name)
...
//_______________________________________________________________________
int EicFRichSubsystem::InitRunSubsystem(PHCompositeNode *topNode)
{
  ...
  // create detector
  m_Detector = new EicFRichDetector(this, topNode, GetParams(), Name());
  m_Detector->OverlapCheck(CheckOverlap());
  // create stepping action if detector is active
  if (GetParams()->get_int_param("active"))
  {
    m_SteppingAction = new EicFRichSteppingAction(m_Detector, GetParams());
  }
  return 0;
}
//_______________________________________________________________________
int EicFRichSubsystem::process_event(PHCompositeNode *topNode)
{
...
//_______________________________________________________________________
void EicFRichSubsystem::Print(const string &what) const
{
  if (m_Detector)
  {
...
//_______________________________________________________________________
PHG4Detector *EicFRichSubsystem::GetDetector(void) const
{
  return m_Detector;
}

//_______________________________________________________________________
void EicFRichSubsystem::SetDefaultParameters()
{
  // sizes are in cm
  // angles are in deg
