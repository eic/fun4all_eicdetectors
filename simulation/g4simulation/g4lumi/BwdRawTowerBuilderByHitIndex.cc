#include "BwdRawTowerBuilderByHitIndex.h"
...
BwdRawTowerBuilderByHitIndex::BwdRawTowerBuilderByHitIndex(const std::string &name)
  : SubsysReco(name)
  , m_Towers(nullptr)
  , m_Geoms(nullptr)
  , m_Detector("NONE")
...
int BwdRawTowerBuilderByHitIndex::InitRun(PHCompositeNode *topNode)
{
...
int BwdRawTowerBuilderByHitIndex::process_event(PHCompositeNode *topNode)
{
  // get hits
...
int BwdRawTowerBuilderByHitIndex::End(PHCompositeNode */*topNode*/)
{
  return Fun4AllReturnCodes::EVENT_OK;
}

void BwdRawTowerBuilderByHitIndex::Detector(const std::string &d)
{
  m_Detector = d;
  m_CaloId = RawTowerDefs::convert_name_to_caloid(m_Detector);
}

void BwdRawTowerBuilderByHitIndex::CreateNodes(PHCompositeNode *topNode)
{
  PHNodeIterator iter(topNode);
  ...
  if (!runNode)
  {
    ...
    throw std::runtime_error("Failed to find Run node in BwdRawTowerBuilderByHitIndex::CreateNodes");
  }
  ...
  if (!dstNode)
  {
    ...
    throw std::runtime_error("Failed to find DST node in BwdRawTowerBuilderByHitIndex::CreateNodes");
  }
...
bool BwdRawTowerBuilderByHitIndex::ReadGeometryFromTable()
{
  ...
  while (getline(istream_mapping, line_mapping))
  {
    ...
    if (line_mapping.find("#") != string::npos)
    {
      if (Verbosity() > 0)
      {
        cout << "BwdRawTowerBuilderByHitIndex: SKIPPING line in mapping file: " << line_mapping << endl;
      }
      continue;
    }
...
//      size_z = 20 * cm;
    ...
    if (line_mapping.find("Bwd ") != std::string::npos)
    {
      ...
      {
        std::cout << "ERROR in BwdRawTowerBuilder: Failed to read global position from  mapping file " << m_MappingTowerFile << std::endl;
        gSystem->Exit(1);
      }
...
//      if (!(iss >> idx_j >> idx_k >> idx_l >> pos_x >> pos_y >> pos_z >> rot_x >> rot_y >> rot_z >> dummy))
      {
        std::cout << "ERROR in BwdRawTowerBuilder: Failed to read line in mapping file " << m_MappingTowerFile << std::endl;
        gSystem->Exit(1);
      }
...
//      temp_geo->set_tower_type((int) type);
	...
	else std::cout <<"ERROR in BwdRawTowerBuilder: Unknown line in Mapping File " <<m_MappingTowerFile << std::endl;
//    }
  ...
  * after reading parameters from file */
    ...
    it != all_towers.second; ++it)
  {
    ...
    if (Verbosity() > 2)
    {
      ...
      cout << "* Tower size x y z : " << it->second->get_size_x() << " " << it->second->get_size_y() << " " << it->second->get_size_z() << endl;
