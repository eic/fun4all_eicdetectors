...
if(m_Params->get_int_param("ispipe")==0){
...
//  m_LogicalVolumesSet.insert(logical);
 //end implement your own here://
  ...
  G4VSolid *solidPipe = new G4Tubs("EICG4B0IonPipe",
        m_Params->get_double_param("pipe_ir") *cm,
        m_Params->get_double_param("pipe_or") *cm,
        m_Params->get_double_param("length") * cm,
        0., 360.*degree);

  G4LogicalVolume *logical = new G4LogicalVolume(solidPipe,
	  G4Material::GetMaterial(m_Params->get_string_param("material")),
          "EICG4B0Logical");

  G4VisAttributes *vis = new G4VisAttributes(G4Color(0.5, 0.5, 0.5, 0.5));
  vis->SetForceSolid(true);
  logical->SetVisAttributes(vis);
  G4RotationMatrix *rotm = new G4RotationMatrix();
  rotm->rotateY(m_Params->get_double_param("rot_y") * deg);
  G4VPhysicalVolume *phy = new G4PVPlacement(
      rotm,
      G4ThreeVector(m_Params->get_double_param("pipe_x") * cm,
                    m_Params->get_double_param("pipe_y") * cm,
                    m_Params->get_double_param("pipe_z") * cm),
      logical, "EICG4B0", logicWorld, 0, false, OverlapCheck());
  m_PhysicalVolumesSet.insert(phy);
  m_PhysicalVolumesDet.insert({phy, m_Params->get_double_param("detid")+1});
  return;
