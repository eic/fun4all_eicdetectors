...
#define LogDebug(exp) \
  if (Verbosity()) std::cout << "B0TrackFastSim (DEBUG): " << __FILE__ << ": " << __LINE__ << ": " << exp << "\n"
#define LogError(exp) \
  std::cout << "B0TrackFastSim (ERROR): " << __FILE__ << ": " << __LINE__ << ": " << exp << "\n"
#define LogWarning(exp) \
  std::cout << "B0TrackFastSim (WARNING): " << __FILE__ << ": " << __LINE__ << ": " << exp << "\n"
...
int B0TrackFastSim::InitRun(PHCompositeNode* topNode)
{
  m_EventCnt = -1;
  ...
  for (std::map<std::string, std::pair<int, double>>::iterator iter = m_ProjectionsMap.begin(); iter != m_ProjectionsMap.end(); ++iter)
  {
    ...
    case DETECTOR_TYPE::Vertical_Plane:
    {
      ...
      if (Verbosity() > 1) std::cout << "B0 Track: x, y, z: " << temp_geo->get_center_x() << " " << temp_geo->get_center_y() << " " << temp_geo->get_center_z() << std::endl;
...
int B0TrackFastSim::End(PHCompositeNode* /*topNode*/)
{
...
int B0TrackFastSim::process_event(PHCompositeNode* /*topNode*/)
{
  m_EventCnt++;

  if (Verbosity() >= 2)
    std::cout << "B0TrackFastSim::process_event: " << m_EventCnt << ".\n";
 ...
 */
bool B0TrackFastSim::FillSvtxVertexMap(
...
int B0TrackFastSim::CreateNodes(PHCompositeNode* topNode)
{
  ...
  //	if (Verbosity() > 0)
  //		cout << _clustermap_out_name <<" node added" << std::endl;
...
int B0TrackFastSim::GetNodes(PHCompositeNode* topNode)
{
  ...
  for (unsigned int i = 0; i < m_PHG4HitsNames.size(); i++)
  {
    ...
    if (!phg4hit)
    {
      std::cout << PHWHERE << m_PHG4HitsNames[i]
    ...
    if (Verbosity() > 0)
    {
      std::cout << "B0TrackFastSim::GetNodes - node added: " << m_PHG4HitsNames[i] << std::endl;
    }
  ...
  if (!runNode)
  {
    ...
    throw std::runtime_error(
        "Failed to find Run node in B0TrackFastSim::CreateNodes");
  }
  if (Verbosity())
  {
                                             ...
                                             TVector3& seed_mom, TMatrixDSym& seed_cov, const bool do_smearing)
{
  ...
  if (Verbosity())
  {
    std::cout << "B0TrackFastSim::PseudoPatternRecognition - DEBUG: "
        ...
        if (hit->get_trkid() == particle->get_track_id() || gsl_ran_binomial(m_RandomGenerator, detnoise, 1) > 0)
        {
          if (gsl_ran_binomial(m_RandomGenerator, dethiteff, 1) > 0)
          {
            ...
            if (dettype == Vertical_Plane)
            {
              if (Verbosity())
              {
                std::cout << "B0TrackFastSim::PseudoPatternRecognition -adding vertical plane hit ilayer: "
            ...
            else if (dettype == Cylinder)
            {
              if (Verbosity())
              {
                std::cout << "B0TrackFastSim::PseudoPatternRecognition -adding cylinder hit ilayer: "
  ...
  for (auto& pair : ordered_measurements)
  {
    ...
    if (Verbosity())
    {
      std::cout << "B0TrackFastSim::PseudoPatternRecognition - measruement at t =  " << pair.first << " ns: ";
  ...
  if (Verbosity())
  {
    std::cout << "B0TrackFastSim::PseudoPatternRecognition - meas_out.size = " << meas_out.size() << " for "
              << "particle: "
              << std::endl;
    particle->identify();

    std::cout << "B0TrackFastSim::PseudoPatternRecognition - seed_pos = "
              << seed_pos.x() << ", " << seed_pos.y() << ". " << seed_pos.z() << std::endl;
    std::cout << "B0TrackFastSim::PseudoPatternRecognition - seed_pos = "
              << seed_mom.x() << ", " << seed_mom.y() << ". " << seed_mom.z() << std::endl;
    std::cout << "B0TrackFastSim::PseudoPatternRecognition - seed_cov = "
...
bool B0TrackFastSim::MakeSvtxTrack(SvtxTrack* out_track,
...
PHGenFit::PlanarMeasurement* B0TrackFastSim::PHG4HitToMeasurementVerticalPlane(
...
PHGenFit::PlanarMeasurement* B0TrackFastSim::PHG4HitToMeasurementCylinder(
    const PHG4Hit* g4hit, const double phi_resolution,
...
PHGenFit::Measurement* B0TrackFastSim::VertexMeasurement(const TVector3& vtx, double dxy, double dz)
{
  TMatrixDSym cov(3);
  cov.Zero();
  cov(0, 0) = dxy * dxy;
...
{
  std::cout << "B0TrackFastSim: Adding state name " << stateName << std::endl;
...
void B0TrackFastSim::add_cylinder_state(const std::string& stateName, const double radius)
{
...
void B0TrackFastSim::add_zplane_state(const std::string& stateName, const double zplane)
{
  std::cout << "B0TrackFastSim: Adding z plane state " << stateName << " " << zplane << std::endl;
  if (reserved_cylinder_projection_names.find(stateName) != reserved_cylinder_projection_names.end() ||
      reserved_zplane_projection_names.find(stateName) != reserved_zplane_projection_names.end())
  {
