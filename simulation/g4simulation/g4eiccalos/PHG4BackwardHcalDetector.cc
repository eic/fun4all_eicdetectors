...
PHG4BackwardHcalDetector::PHG4BackwardHcalDetector(PHG4Subsystem* subsys, PHCompositeNode* Node, PHParameters* parameters, const std::string& dnam)
  : PHG4Detector(subsys, Node, dnam)
  , m_DisplayAction(dynamic_cast<PHG4BackwardHcalDisplayAction*>(subsys->GetDisplayAction()))
  , m_Params(parameters)
...
//_______________________________________________________________________
int PHG4BackwardHcalDetector::IsInBackwardHcal(G4VPhysicalVolume* volume) const
{
...
//_______________________________________________________________________
void PHG4BackwardHcalDetector::ConstructMe(G4LogicalVolume* logicWorld)
{
  if (Verbosity() > 0)
  {
    std::cout << "PHG4BackwardHcalDetector: Begin Construction" << std::endl;
  }

  if (m_Params->get_string_param("mapping_file").empty())
  {
    cout << "ERROR in PHG4BackwardHcalDetector: No mapping file specified. Abort detector construction." << endl;
...
//_______________________________________________________________________
G4LogicalVolume*
PHG4BackwardHcalDetector::ConstructTower()
{
  if (Verbosity() > 0)
  {
    std::cout << "PHG4BackwardHcalDetector: Build logical volume for single tower..." << std::endl;
  }
  ...
  if (Verbosity() > 0)
  {
    std::cout << "PHG4BackwardHcalDetector: Building logical volume for single tower done." << std::endl;
  }

  return single_tower_logic;
}

int PHG4BackwardHcalDetector::PlaceTower(G4LogicalVolume* hcalenvelope, G4LogicalVolume* singletower)
{
  ...
  for (std::map<std::string, towerposition>::iterator iterator = m_TowerPostionMap.begin(); iterator != m_TowerPostionMap.end(); ++iterator)
  {
    if (Verbosity() > 0)
    {
      std::cout << "PHG4BackwardHcalDetector: Place tower " << iterator->first
...
int PHG4BackwardHcalDetector::ParseParametersFromTable()
{
  ...
  {
    std::cout << "ERROR in PHG4BackwardHcalDetector: Failed to open mapping file " << m_Params->get_string_param("mapping_file") << std::endl;
    gSystem->Exit(1);
  }
  ...
  while (getline(istream_mapping, line_mapping))
  {
    ...
    if (line_mapping.find("#") != string::npos)
    {
      if (Verbosity() > 0)
      {
        std::cout << "PHG4BackwardHcalDetector: SKIPPING line in mapping file: " << line_mapping << std::endl;
      }
      continue;
    }
    ...
    if (line_mapping.find("Tower ") != string::npos)
    {
      ...
      {
        cout << "ERROR in PHG4BackwardHcalDetector: Failed to read line in mapping file " << m_Params->get_string_param("mapping_file") << std::endl;
        gSystem->Exit(1);
      }
