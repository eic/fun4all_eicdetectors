...
PHG4ECAPToFSteppingAction::PHG4ECAPToFSteppingAction(PHG4ECAPToFDetector* detector, const PHParameters* parameters)
  ...
  , m_Zmin(m_Params->get_double_param("z_begin") * cm)
  , m_Zmax(m_Params->get_double_param("z_begin") * cm + 3.0)
{
  m_Zmin -= copysign(m_Zmin, 1. / 1e6 * cm);
  m_Zmax += copysign(m_Zmax, 1. / 1e6 * cm);
}

PHG4ECAPToFSteppingAction::~PHG4ECAPToFSteppingAction()
{
  delete m_Hit;
}

bool PHG4ECAPToFSteppingAction::UserSteppingAction(const G4Step* aStep, bool)
{
  ...
  if (!m_Detector->IsInToF(volume))
  {
    return false;
  }

  int whichactive = m_Detector->IsInToF(volume);
...
void PHG4ECAPToFSteppingAction::SetInterfacePointers(PHCompositeNode* topNode)
{
  std::string hitnodename;
  //string absorbernodename;
  std::string gasnodename;
  ...
  if (!m_HitContainer)
  {
    std::cout << "PHG4ECAPToFSteppingAction::SetTopNode - unable to find " << hitnodename << std::endl;
  }
  // if (!m_AbsorberHits)
  if (!m_ActiveGasHits)
  {
    if (Verbosity() > 1)
    {
      std::cout << "PHG4ECAPToFSteppingAction::SetTopNode - unable to find " << gasnodename << std::endl;
    }
  }
}
