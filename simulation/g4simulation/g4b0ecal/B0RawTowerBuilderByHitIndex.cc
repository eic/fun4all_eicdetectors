...
bool B0RawTowerBuilderByHitIndex::ReadGeometryFromTable()
{
  ...
  while (getline(istream_mapping, line_mapping))
  {
    ...
    if (line_mapping.find("#") != string::npos)
    {
      if (Verbosity() > 0)
      {
        cout << "B0RawTowerBuilderByHitIndex: SKIPPING line in mapping file: " << line_mapping << endl;
      }
      continue;
    }
...
//      temp_geo->set_tower_type((int) type);
	...
	else std::cout <<"ERROR in B0RawTowerBuilder: Unknown line in Mapping File " <<m_MappingTowerFile << std::endl;
//    }
