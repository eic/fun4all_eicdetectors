...
EICG4B0ECALDetector::EICG4B0ECALDetector(PHG4Subsystem *subsys,
  ...
  , m_Layer(lyr)
  , _mapping_tower_file("")
  , m_TowerLogicNamePrefix("B0ECALTower")
{
}
...
//_______________________________________________________________
void EICG4B0ECALDetector::ConstructMe(G4LogicalVolume *logicWorld)
{
  //begin implement your own here://
...
//    gSystem->Exit(1);
  ...
  b0_ecal_log->SetVisAttributes(vis);
...
G4LogicalVolume* EICG4B0ECALDetector::ConstructTower()
{
  if (Verbosity() > 0)
  {
    std::cout << "EICG4B0ECALDetector: Build logical volume for single tower..." << std::endl;
  }
...
int EICG4B0ECALDetector::PlaceTower(G4LogicalVolume* b0ecalenvelope, G4LogicalVolume* singletower)
{
  ...
  for (std::map<std::string, towerposition>::iterator iterator = m_TowerPositionMap.begin(); iterator != m_TowerPositionMap.end(); ++iterator)
  {
    if (Verbosity() > 0)
    {
      std::cout << "EICG4B0ECALDetector: Place tower " << iterator->first
...
int EICG4B0ECALDetector::ParseParametersFromTable()
{
  ...
  {
    std::cout << "ERROR in EICG4B0ECALDetector: Failed to open mapping file " << _mapping_tower_file << std::endl;
    gSystem->Exit(1);
  }
  ...
  while (getline(istream_mapping, line_mapping))
  {
    ...
    if (line_mapping.find("#") != std::string::npos)
    {
      if (Verbosity() > 0)
      {
        std::cout << "EICG4B0ECALDetector: SKIPPING line in mapping file: " << line_mapping << std::endl;
      }
      continue;
    }
...
//_______________________________________________________________
void EICG4B0ECALDetector::Print(const std::string &what) const
{
  std::cout << "EICG4B0ECAL Detector:" << std::endl;
