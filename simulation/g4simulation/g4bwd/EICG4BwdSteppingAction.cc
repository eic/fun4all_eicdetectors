...
//____________________________________________________________________________..
EICG4BwdSteppingAction::EICG4BwdSteppingAction(EICG4BwdSubsystem *subsys, EICG4BwdDetector *detector, const PHParameters *parameters)
...
//____________________________________________________________________________..
EICG4BwdSteppingAction::~EICG4BwdSteppingAction()
{
...
bool EICG4BwdSteppingAction::UserSteppingAction(const G4Step *aStep, bool was_used)
{
...
//        pp->GetShower()->add_g4hit_id(m_SaveHitContainer->GetID(), m_Hit->get_hit_id());
  ...
  else
  {
    if (!m_BlackHoleFlag)
    {
       eion = edep - aStep->GetNonIonizingEnergyDeposit() / GeV;
...
//____________________________________________________________________________..
int EICG4BwdSteppingAction::FindTowerIndexFromPosition(G4StepPoint* prePoint, int& j, int& k)
{
  int j_0 = 0;  //The j and k indices for the scintillator / tower
  int k_0 = 0;  //The j and k indices for the scintillator / tower
  double towersize = m_Params->get_double_param("tower_size");
  double width = m_Params->get_double_param("width");
  double height = m_Params->get_double_param("height");
  double det_x_pos = m_Params->get_double_param("global_x");
  double det_y_pos = m_Params->get_double_param("global_y");
  j_0 = (int) ( ( width - det_x_pos + towersize + ( prePoint->GetPosition().x() / cm ) ) / towersize ); //TODO DRCALO TOWER SIZE
  k_0 = (int) ( ( height - det_y_pos + towersize + ( prePoint->GetPosition().y() / cm ) ) / towersize ); //TODO DRCALO TOWER SIZE
  j = (j_0 * 1);
  k = (k_0 * 1);
...
//____________________________________________________________________________..
void EICG4BwdSteppingAction::SetInterfacePointers(PHCompositeNode *topNode)
{
  m_HitContainer = findNode::getClass<PHG4HitContainer>(topNode, m_HitNodeName);
  if (!m_HitContainer)
  {
    std::cout << "EICG4BwdSteppingAction::SetTopNode - unable to find "
              << m_HitNodeName << std::endl;
	gSystem->Exit(1);
  }
}
bool EICG4BwdSteppingAction::hasMotherSubsystem() const
{
