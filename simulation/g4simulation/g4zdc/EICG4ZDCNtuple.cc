// - 1/June/2021 Ntuple production for ZDC G4 hits     Shima Shimizu
//
#include "EICG4ZDCNtuple.h"
...
EICG4ZDCNtuple::EICG4ZDCNtuple(const std::string &name, const std::string &filename) 
  : SubsysReco(name)
  , nblocks(0)
  , hm(nullptr)
  , _filename(filename)
...
int EICG4ZDCNtuple::Init(PHCompositeNode *)
 {
   ...
   ntup = new TNtuple("zdchit", "ZDCs", "detid:layer:xid:yid:x0:y0:z0:x1:y1:z1:t0:t1:edep");
   //  ntup->SetDirectory(0);
   return 0;
 }
 ...
 void EICG4ZDCNtuple::AddNode(const std::string &name, const int detid)
 {
